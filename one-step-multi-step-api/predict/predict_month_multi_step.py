from .utilities.preprocessing import preprocess_data_month
from .utilities.feature_engineering import feature_engineering_month
...
def run_predict_month_multi_step(df):
    script_dir = os.path.dirname(__file__)
    rel_path = "models//month_multi_step.joblib"
    abs_file_path = os.path.join(script_dir, rel_path)
    model = joblib.load(abs_file_path)
    df = preprocess_data_month(df)
    df = feature_engineering_month(df)
