...
def train_model(X_train, y_train):
    model = RandomForestRegressor(
        n_estimators=700, max_depth=6, min_samples_leaf=2, random_state=42
    )
...
def main(args):
    ...
    joblib.dump(model, args.model_save_path + "rf_multistep_week.joblib")
    test_data.to_csv("data/evaluation/test_multistep_week.csv", index=False)
